...
class RLtraining(Node):
    def __init__(self):
        ...
        """
        self.agent = DDPGAgent.DDPGAgent()
        ...
        # 임시 모델/버퍼 로드
        model_temp_path  = os.path.join(self.results_dir, "RL_model_temp.pth")
        if os.path.exists(model_temp_path):
            self.agent.load_model(path=model_temp_path)
            print(f"Loaded :     {model_temp_path}")
        buffer_temp_path = os.path.join(self.results_dir, "RL_buffer_temp.pkl")
        if os.path.exists(buffer_temp_path):
            with open(buffer_temp_path, "rb") as f:
                buf_list = pickle.load(f)
            self.agent.buffer.buffer = deque(buf_list, maxlen=self.agent.buffer.buffer.maxlen)
            print(f"Loaded :     {buffer_temp_path}     /     ({len(buf_list)} transitions)")
            ...
            if self.state == 'Tagging':
                ...
                if self.done_episode:
                    if self.episode_number == self.max_episodes:
                        final_name = f"RL_model_final_episode{self.episode_number}.pth"
                        final_path = os.path.join(self.results_dir, final_name)
                        self.agent.save_model(path=final_path)
                    elif self.episode_number % 5 == 0:
                        ep10_name = f"RL_model_episode{self.episode_number}.pth"
                        ep10_path = os.path.join(self.results_dir, ep10_name)
                        self.agent.save_model(path=ep10_path)

                    model_temp_path = os.path.join(self.results_dir, "RL_model_temp.pth")
                    self.agent.save_model(path=model_temp_path)
                    print(f"Saved :      {model_temp_path}")
                
                    buffer_temp_path = os.path.join(self.results_dir, "RL_buffer_temp.pkl")
                    with open(buffer_temp_path, "wb") as f:
                        pickle.dump(list(self.agent.buffer.buffer), f)
                    print(f"Saved :      {buffer_temp_path}     /     ({len(self.agent.buffer)} transitions)")
                ...
                if self.latest_bbox is not None:
                    ...
                    error_vertical_normalized = error_vertical / 1.0
                    error_forward_normalized = error_forward / 5.0
                    error_lateral_normalized = error_lateral / 300.0

                    current_step_state = np.array([error_vertical_normalized, error_forward_normalized, error_lateral_normalized])
                    speed = np.linalg.norm(self.vel)

                    reward = self.agent.compute_reward(error_vertical_normalized, error_forward_normalized, error_lateral_normalized, speed, self.pitch)
                    
                    if self.last_step_state is not None and self.last_step_action is not None:
                        self.agent.remember(self.last_step_state, self.last_step_action, reward, current_step_state, self.done_episode)
                    ...
                    avg_reward_episode = sum(self.episode_rewards) / len(self.episode_rewards) if self.episode_rewards else 0.0
                    ...
                    if self.env_step % 100 == 0:
                        ...
                        with open(csv_path, 'a', newline='') as f:
                            writer = csv.writer(f)
                            if not file_exists:
                                writer.writerow(["episode", "env_step", "train_step", "avg_reward_100_env_step", "avg_reward_episode"])
                            writer.writerow([self.episode_number, self.env_step, self.train_step, f"{avg_reward_recent:.4f}", f"{avg_reward_episode:.4f}"])

                        if avg_reward_episode >= -2.5:
