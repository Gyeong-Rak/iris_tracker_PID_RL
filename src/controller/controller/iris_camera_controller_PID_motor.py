    ...
    def main_callback(self):
        ...
        if self.state == 'Tagging':
            ...
            if self.latest_bbox is not None:
                ...
                motor_speeds = [0.8, 0.8, 0.8, 0.8] # 0.8 to hover
                self.publish_motor_command(motor_speeds)
            ...
            correction_weight = np.array([0.005, 0.0001, 0.02]) # [vertical, forward, yaw]

            correction_vertical = np.clip(self.pid_vertical.update(error_vertical), -1.5, 1.5) / 1.5 * correction_weight[0]
            correction_forward = np.clip(self.pid_forward.update(error_forward), -3, 3) / 3 * correction_weight[1]
            correction_yaw = np.clip(self.pid_lateral.update(error_lateral), -0.5, 0.5) / 0.5 * correction_weight[2]

            print(f"ver_cor:{correction_vertical:0f}, for_cor:{correction_forward:0f}, lat_cor:{correction_yaw:0f}")

            motor_speeds = [0.8 + correction_vertical - correction_forward - correction_yaw,  # forward right
                            0.8 + correction_vertical + correction_forward - correction_yaw,  # backward left
                            0.8 + correction_vertical - correction_forward + correction_yaw,  # forward left
                            0.8 + correction_vertical + correction_forward + correction_yaw]  # backward right
            
            if correction_forward == 3 and correction_vertical > 1: motor_speeds = [0.9, 0.95, 0.9, 0.95] # 0.8 to hover
