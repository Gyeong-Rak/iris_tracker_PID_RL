...
class IrisCameraController(Node):
    def __init__(self):
        ...
        self.dt = 0.05
        self.pid_forward = PID(Kp=3, Ki=0.2, Kd=0.0, dt=self.dt)
        self.pid_lateral = PID(Kp=2.7, Ki=3, Kd=3, dt=self.dt)
        ...
        filename = f"error_log_{start_time}_PID.csv"
        ...
        try:
            ...
            if self.state == 'Tagging':
                ...
                if self.latest_bbox is not None:
                    ...
                    # --- PID ---
                    correction_vertical = np.clip(self.pid_vertical.update(error_ver_norm), -1, 1)
                    correction_forward = np.clip(self.pid_forward.update(error_fwd_norm), -2, 2)
                    correction_yaw = np.clip(self.pid_lateral.update(error_lat_norm), -0.5, 0.5)
